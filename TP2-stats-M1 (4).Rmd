...
# Dans un premier temps, nous voulions étudier notre source de données en prenant des statistiques générale afin d'avoir une idée globale sur notre dataset. Pour cela nous avons commencé par charger notre dataset et par la suite nous nous sommes servis des fonctions telles dim et summary qui nous ont données des informations telles (La moyenne, la médiane, le min et le max, nombre de ligne et colonnes .. )

```{r}
# Chargement de notre source de données
iris <- read.csv("C:/Users/HomePC/Downloads/iris.csv", sep=";")
...
# Distribution des variables : Nous avons utlisé des histogrammes afin de visualiser la ditribution de chaqu'une de nos variables.
  
```{r}
# Distribution de la variable Sepal.length
hist(iris[, 'Sepal.Length'], main= 'Distribution Longeur Sepal de toutes les fleurs', xlab='Length (cm)')

# Distribution de la variable Sepal.Width
hist(iris[, 'Sepal.Width'], main= 'Distribution Largeur Sepal de toutes les fleurs', xlab='Width (cm)')

# Distribution de la variable Petal.Length
hist(iris[, 'Petal.Length'], main= 'Distribution Longeur Sepal de toutes les fleurs', xlab='Length (cm)')

# Distribution de la variable Petal.width
hist(iris[, 'Petal.Width'], main= 'Distribution Largeur Petal de toutes les fleurs', xlab='Width (cm)')


```

# Distribution des variables par espèce: Nous avons répartis par type de variable (Sepal.Width, Sepal.Length...) nos données dans le but de voir comment chaqu'une des variables soit distribuée avec la caractéristique catégorique "Espèce".
...
ggplot(iris_sepal_width_feature, aes(x= value, fill=Species)) + geom_histogram(bins=20, color='white', position='identity', alpha=0.75) + ggtitle('Distribution de Sepal Width par espèce') + facet_wrap(.~Species, nrow = 1) + theme(strip.text.x = element_text(size = 22))


# Sapel.Length par espèce
ggplot(iris_sepal_length_feature, aes(x= value, fill=Species)) + geom_histogram(bins=20, color='white', position='identity', alpha=0.75) + ggtitle('Distribution de Sepal Length par espèce') + facet_wrap(.~Species, nrow = 1) + theme(strip.text.x = element_text(size = 22))


# Petal.Length par espèce
ggplot(iris_petal_length_feature, aes(x= value, fill=Species))  + geom_histogram(bins=20, color='white', position='identity', alpha=0.75) + ggtitle('Distribution de Petal Length par espèce') + facet_wrap(.~Species, nrow = 1) + theme(strip.text.x = element_text(size = 22))


# Petal.Width par espèce
ggplot(iris_petal_width_feature, aes(x= value, fill=Species)) + geom_histogram(bins=20, color='white', position='identity', alpha=0.75) + ggtitle('Distribution de Petal Width par espèce') + facet_wrap(.~Species, nrow = 1) + theme(strip.text.x = element_text(size = 22))
...
Les variables qui semblent les plus pertinentes pour l'ACP sont les moins correlées (Sepal.length et Sepal.width)
...
round(eig.val,2)
# Afin d'observer le graphique des valeurs propres on a executé la commande ci-dessous
...
```
Le cercle des corrélations est un cercle, de rayon 1 où l’axe des abscisses représente le premier axe factoriel selon ~v1 et l’axe des ordonnées le second axe.
...
Afin de voir plus préciseamment les coordonnées précises des composantes principales :

```{r}
round(get_pca_var(iris.pca)$coord, 2)
```
On peut remarquer que Les petals et Sepal.Length sont très corrélées avec le premier axe factoriel. Cela signifie que les sepals et largeur de Petal sont très liées aux valeurs sur l'axe 1. Pour Sepal.Width, il est liée aux valeurs du deuxieme axe.
...
Notre algorithm peut avoir une limite de mieux différencier entre la classification versicolor et virginica vu que dans  # A revoir
...
Comme on peut remarquer que pour la dimension 1 explique 92.46 de notre jeu de données, plus que l'acp.
...
Remarque: sur ce graphique on peut remarquer que la première dimension explique une grande partie de l'information de notre nuage de points avec un pourcentage de 92.5%

En terme de variance cumulée , les deux premières dimensions expliquent 97.7% de l'information de notre jeu de données 
...
```{r}
# voir plus préciseamment les coordonnées précises des composantes principales
...
La méthode qui semble donner les meilleurs résultats est celle de l'acp non réduite car comme on avait vu avant que la dimension 1 seule explique 92.5% en Acp non réduite contrairement à l'ACP où explique que 73%.
...
```{r}
# Distribution de la variable sleep_total
hist(sleep[, 'sleep_total'], main= 'Distribution de la variable Sleep Total', xlab='Hour')

# Distribution de la variable sleep_rem
hist(sleep[, 'sleep_rem'], main= 'Distribution de la variable Sleep Rem', xlab='Hour')

# Distribution de la variable awake
hist(sleep[, 'awake'], main= 'Distribution de la variable Awake', xlab='Hour')

# Distribution de la variable brain_wt
hist(sleep[, 'brain_wt'], main= 'Distribution de la variable Brain Wt', xlab='Kg')

# Distribution de la variable body_wt
hist(sleep[, 'body_wt'], main= 'Distribution de la variable Body Wt', xlab='Kg')

# Distribution de la variable brain_body_ratio
hist(sleep[, 'brain_body_ratio'], main= 'Distribution de la variable Brain Body Ratio', xlab='')

# Distribution de la variable gest_day
hist(sleep[, 'gest_day'], main= 'Distribution de la variable Gest Day', xlab='Période')
```
>> - Corrélation entre les variables.
...
# Les varibales qui semblent les plus pertinentes pour l'ACP sont brain_body_ration, body_wt, sleep_rm
...
```{r}
library(tidyverse)
sleep <- as.tibble(sleep)
as.tibble(sleep)

library(car)
scatterplot(sleep_rem~awake, data=sleep)

prest.lm1 <- lm(sleep_rem~awake, data=sleep)

#my_df <- data.frame(awake=c(12.00))
#predict(prest.lm1, newdata=my_df)

for(i in 1:nrow(sleep)){
  if(is.na(sleep$sleep_rem[i])){
    my_df <- data.frame(awake=c(sleep$awake[i]))
    sleep$sleep_rem[i]<-predict(prest.lm1, newdata=my_df)
  }
}
...
library("FactoMineR")
library("factoextra")
sleep.pca <- PCA(sleep[,5:11], graph = FALSE)
...
Les 3 première variables semblent plus pertinentes pour l'ACP car en terme de la variance cumulée, ces trois dimensions expliquent presque 90% de l'information de notre jeu de données.
